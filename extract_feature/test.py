        ...
        try:
            with tf.device(self.device):
                if fov == 'v' or fov =='video' or fov =='videos':
                    ...
                    logging.error("Error on _process function: INPUT TYPE ERROR")
    ...
    def __process_video(self):
        ...
        for idx,video in enumerate(videoName):
            ...
            if (vidcap.isOpened()== False):
                #check opened?
                with open(os.path.join(self.output,'Failed.txt'),'a') as file:
                    file.write(video+'\n')
                continue
...
def main():
    #Example for runing
    outputpath = '/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/TVSum_processed_data/frames_feature/inceptionv3'
    ex = ExtractFeature('tvsum',outputpath,sampling_rate=1,device_name='GPU:0').InceptionV3()
